...
/**
 * Test Garble() by comparing with REFERENCE from lib_server(ie golden data)
 */
TEST(GarbleReferenceTest, Adder) {
  GarbledCircuit garbled_circuit;
  test::GetFullAdder(&garbled_circuit);

  garbled_circuit.Garble(123456789);

  garble::ParallelGarbledCircuit parallel_garbled_circuit(
